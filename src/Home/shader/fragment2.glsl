...
void main()	{
	...
	sin(position.x * 4. + progress*3. + time)*mix(0.3,.1, abs(0.5-position.x))*0.5*smoothstep(0.,0.2,progress)
	);
    ...
    vec3 pink = vec3(0.8, 0.2, 0.4);
    float gradientFactor = smoothstep(0.4, 0.1, vUv.y + trans - progress);
    vec3 redpinkGradient = mix(red, pink, gradientFactor);

    color.rgb = mix(color.rgb, redpinkGradient, 0.9 * smoothstep(0.5, 0.001, abs(0.5 - color.a)));
